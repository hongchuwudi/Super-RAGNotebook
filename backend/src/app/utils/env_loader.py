...
def load_backend_env(backend_dir: str | Path | None = None) -> bool:
    """Load backend/.env only for manual backend runs.

    start.py injects the root .env into child process environments and sets
    RAGNOTEBOOK_ENV_INJECTED=1. In that mode backend/.env must not be read.
    """
    if is_env_injected():
        return False
